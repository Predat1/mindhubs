...
CREATE TABLE IF NOT EXISTS public.vendor_subscriptions (
  ...
  status           TEXT NOT NULL DEFAULT 'active'
                   CHECK (status IN ('active','past_due','cancelled','trialing','pending')),
...
-- Nettoyage et insertion des limites de plan
DELETE FROM public.plan_limits;
INSERT INTO public.plan_limits VALUES
  ('free',    1,    50,   0.100, 0,      0,       false, false, false, false, NULL),
  ('starter', 3,    200,  0.100, 4900,   49000,   false, false, false, false, NULL),
  ('pro',     20,   800,  0.070, 14900,  149000,  true,  true,  false, false, 'Vendeur Pro'),
  ('elite',   -1,   2500, 0.050, 29900,  299000,  true,  true,  true,  true,  'Vendeur Elite');
...
-- Politiques RLS (vérifier si has_role existe, sinon utiliser une version simplifiée ou la créer)
...
-- 7. Fonction SQL sécurisée pour dépenser des crédits
...
BEGIN
  ...
  UPDATE vendor_credits
  SET balance = v_credits + v_rollover,
      total_earned = total_earned + v_credits,
      updated_at = now()
  WHERE vendor_id = p_vendor_id;
  ...
  VALUES
    (p_vendor_id, 'monthly_grant', v_credits, v_credits + v_rollover,
     'Crédits mensuels ' || v_plan || ' + ' || v_rollover || ' rollover');
END;
$$;

-- 9. Bloquer ajout produit si limite atteinte
...
-- Initialisation auto des crédits pour les vendeurs existants sans compte crédits
INSERT INTO vendor_credits (vendor_id, balance)
SELECT id, 50 FROM vendors
WHERE id NOT IN (SELECT vendor_id FROM vendor_credits)
