...
const Inner = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && (parsed.title || parsed.description || parsed.image_url)) {
          setForm(parsed);
          setDraftRestored(true);
          toast.info("Brouillon restauré", {
            description: "Vos modifications non sauvegardées ont été récupérées.",
          });
        }
      }
    } catch {/* ignore */}
  ...
  const generateAIFeatures = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("generate-product-features", {
        body: { title: form.title, description: form.description, category: form.category },
      });
      ...
      setForm((f) => ({ ...f, key_features: [...f.key_features, ...features] }));
  ...
  const editAIImage = async () => {
    if (!form.image_url) {
      toast.error("Aucune image", { description: "Ajoutez d'abord une image." });
      return;
    }
    if (editPrompt.trim().length < 3) {
      toast.error("Prompt requis", { description: "Décrivez la modification souhaitée." });
      return;
    }
    if (!user) return;
  ...
  const handleSave = async (overrideStatus?: "draft" | "published") => {
    ...
    if (finalStatus === "published" && !allValid) {
      toast.error("Champs requis manquants", { description: "Pour publier : titre, image et prix obligatoires." });
      return;
    }
    if (!form.title.trim()) {
      toast.error("Titre requis", { description: "Même un brouillon a besoin d'un titre." });
      return;
    }
    setSaving(true);
    try {
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 transition ${
              isDraft ? "border-amber-500/40 bg-amber-500/10" : "border-green-500/40 bg-green-500/10"
            }`}>
              <FileEdit size={12} className={isDraft ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground/50"} />
              <Switch
                checked={!isDraft}
                onCheckedChange={(v) => setForm((f) => ({ ...f, status: v ? "published" : "draft" }))}
                aria-label="Publié"
              />
              <Globe size={12} className={!isDraft ? "text-green-600 dark:text-green-400" : "text-muted-foreground/50"} />
              <span className={`text-[10px] font-bold uppercase tracking-wide ${
                isDraft ? "text-amber-700 dark:text-amber-400" : "text-green-700 dark:text-green-400"
              }`}>
            ...
            <Button size="sm" onClick={() => handleSave("published")} disabled={saving || !allValid}>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 animate-fade-in" key={step}>
              ...
              {step === "media" && (
                <div className="space-y-5">
                  ...
                  {/* Style picker for AI generation */}
                  <div className="space-y-2 rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5 p-3">
                    <div className="flex items-center gap-2">
                      <Sparkles size={14} className="text-primary" />
                      <p className="text-xs font-bold text-foreground">Génération IA — Style</p>
                    </div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {currentStepIndex < STEPS.length - 1 ? (
                ...
                <Button onClick={() => handleSave("published")} disabled={saving || !allValid}>
                  {saving ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}
      ...
      <Dialog open={showVariants} onOpenChange={setShowVariants}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {aiImgLoading && variants.length === 0 ? (
              ...
              variants.map((url, i) => (
                ...
                >
                  <img src={url} alt={`Variante ${i + 1}`} className="h-full w-full object-cover transition group-hover:scale-105" />
                  <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/80 to-transparent p-2">
                    <span className="text-[10px] font-bold text-white">Variante {i + 1}</span>
          ...
          <DialogFooter className="sm:justify-between">
            <Button variant="outline" size="sm" onClick={() => setShowVariants(false)}>
              Annuler
            </Button>
            <Button
              size="sm"
              ...
              className="gap-1.5"
            >
              {aiImgLoading ? <Loader2 className="animate-spin" size={13} /> : <RefreshCw size={13} />}
              Régénérer
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Décrivez la modification souhaitée. L'IA va éditer l'image existante.
            </DialogDescription>
          </DialogHeader>

          {form.image_url && (
            <div className="overflow-hidden rounded-lg border border-border">
              <img src={form.image_url} alt="Image actuelle" className="h-40 w-full object-contain bg-muted/30" />
            </div>
          )}
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={editAIImage} disabled={aiEditLoading || !editPrompt.trim()}>
              {aiEditLoading ? <Loader2 className="animate-spin" size={14} /> : <Wand2 size={14} />}
              Modifier
            </Button>
          </DialogFooter>
        </DialogContent>
