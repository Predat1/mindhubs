...
const CartPage = () => {
  ...
  // Cross-sell: products from same categories not already in cart
        ...
        <div className="grid lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
              ...
              <AnimateOnScroll key={item.product.id} delay={idx * 100}>
                <div className="stat-card rounded-xl p-4 flex gap-4 items-center">
                  <Link to={`/produit/${item.product.id}`}>
                    <img src={item.product.image} alt={item.product.title} className="w-20 h-20 rounded-lg object-cover shrink-0 hover-scale" />
                  </Link>
                  <div className="flex-1 min-w-0 space-y-1">
                    <Link to={`/produit/${item.product.id}`}>
                      <h3 className="text-foreground font-semibold text-sm truncate hover:text-primary transition-colors">{item.product.title}</h3>
                    </Link>
                  ...
                  <button onClick={() => removeFromCart(item.product.id)} className="text-muted-foreground hover:text-destructive transition-colors p-2 hover-scale">
                    <Trash2 size={18} />
                  </button>
                </div>
              </AnimateOnScroll>
          ...
          <AnimateOnScroll delay={200}>
            <div className="stat-card rounded-2xl p-6 space-y-6 sticky top-24">
              ...
              <Link to="/checkout" className="block w-full btn-primary-brand py-3 rounded-full font-semibold text-sm tracking-wide hover-scale text-center shadow-glow">
                COMMANDER
              </Link>
          ...
          <div className="max-w-5xl mx-auto">
            <AnimateOnScroll>
              <div className="flex items-center gap-4 mb-6">
                <h2 className="text-lg font-bold text-foreground whitespace-nowrap">Complétez votre formation</h2>
